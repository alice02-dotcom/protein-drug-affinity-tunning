...
#aaindex위한 함수 호출
from protein_feature_builder import build_protein_representation, AAindexFeatureEmbedder, AAINDEX_DICT, AAINDEX_KEYS
...
class EntryDataset(InMemoryDataset):
    def __init__(self, root_folder,transform=None, pre_transform=None,
        ...
        if os.path.exists(self.processed_paths[0]):
            ...
            #self.process()  #.pt없으면 자동으로 process실행
    ...
    def protein_process(self,protein_df,pdb_graph_dict,flag_add_self_loops=False, default_dim_features=1,
        ...
        embedder = AAindexFeatureEmbedder(AAINDEX_DICT, AAINDEX_KEYS)
        seq_dict = dict(zip(protein_df['targetID'].values, protein_df['sequences'].values))
        ...
        for unp_id in tqdm(self.unpIDs) :
            ...
            #AAindex와 구조 기반 feature concat 코드 추가----
            sequence = seq_dict.get(unp_id, None)
            if sequence is None:
                print(f"[WARNING] sequence not found for {unp_id}, skipping.")
                continue

            try:
                x = build_protein_representation(sequence, feat_mat, embedder=embedder, normalize=True)
                print(f"[DEBUG] Final feature shape for {unp_id}: {x.shape}")
            except Exception as e:
                print(f"[ERROR] failed to build feature for {unp_id}: {e}")
                continue
            #------------------------------------------------
            #디버깅용 코드 추가-------------------------------
            try:
                aaindex_feat = embedder(sequence)
                print(f"[DEBUG] AAindex shape: {aaindex_feat.shape}")  # <== AAindex feature shape
                print(f"[DEBUG] Fingerprint shape: {feat_mat.shape}")  # <== 구조 기반 feature shape
                x = build_protein_representation(sequence, feat_mat, embedder=embedder, normalize=True)
                print(f"[DEBUG] Final node feature shape after concat: {x.shape}")
            except Exception as e:
                print(f"[ERROR] failed to build feature for {unp_id}: {e}")
                continue
            #-----------------------------------------------
        ...
        self.data = data
        self.slices = slices
        self.entryIDs = self.entryIDs
        self.unpIDs = self.unpIDs
        #-------------------
    ...
    def my_prepare_data(self):
        ...
        if self.entry2_data_folder is not  None:
            ...
            if os.path.isfile(self.entry2_data_folder ):
                ...
                #추가 삽입 코드---------------
            if not os.path.exists(self.entry2_dataset.processed_paths[0]):
                print('[INFO] Processed .pt file not found. Running protein_process...')
                import pickle
                protein_df = pd.read_csv(self.entry2_seq_file)

                # pdb_graph_dict 경로는 프로젝트 구조에 맞게 수정하세요
                with open('./dataset/KIBA_subset/target/processed/pdb_graph_dict.pkl', 'rb') as f:
                    pdb_graph_dict = pickle.load(f)

                self.entry2_dataset.protein_process(protein_df, pdb_graph_dict)
                #----------------------------
